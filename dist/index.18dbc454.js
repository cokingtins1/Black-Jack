...
createDeck();
function createDeck() {
    for(let s = 0; s < suits.length; s++)for(let v = 0; v < values.length; v++)singleDeck.push(values[v] + suits[s]);
}
let deck = [];
const createBlack8 = (arr, n)=>[].concat(...Array(n).fill(arr));
...
},{}]},["gEwwu","1SICI"], "1SICI", "parcelRequirefe86")
